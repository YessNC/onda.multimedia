...
as $$
  ...
  base_slots as (
    ...
    join public.availability_rules rule
      ...
      and rule.weekday = extract(dow from preview_dates.booking_date)::integer
  ...
  exception_slots as (
    select
      exception.exception_date as booking_date,
      generated.slot_start,
      generated.slot_start + (service_options.duration_minutes * interval '1 minute') as slot_end,
      service_options.service_id,
      service_options.service_name,
    ...
    join public.availability_exceptions exception
      ...
      and exception.exception_date >= (select from_date from input)
      and exception.exception_date < (select input.from_date + input.days from input)
    ...
    cross join lateral generate_series(
      exception.exception_date + exception.start_time,
    ...
    where not (
      ...
      and not exists (
        ...
        where blocked.type = 'blocked'
          and blocked.exception_date = candidate_slots.booking_date
